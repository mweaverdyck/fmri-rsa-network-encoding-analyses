...
for pred in "${preds[@]}"; do
  ...
  for corr in "${corrs[@]}"; do
    ...
    for dist in "${dists[@]}"; do
      ...
      out_t_test=${out_dir}/${dist}/${fname_root}_mask-${MASK_NAME}_smooth-${SMOOTH_FWMH}_vsmooth-${VAR_SMOOTH}_nperm-${N_PERMS}_clust-${CLUSTER_THRESH} #_vthresh-${vox_thresh}
      # http://web.mit.edu/fsl_v5.0.10/fsl/doc/wiki/Randomise(2f)UserGuide.html
      randomise -i $img_4D_smooth -m ${MASK} -o $out_t_test -1 -v ${VAR_SMOOTH} -T -x -n ${N_PERMS} -C ${CLUSTER_THRESH} --uncorrp | tee -a $log_file # -C ${vox_thresh}
