...
fmriprep ${BIDS_DIR} "${out_dir}/.." --work-dir "${work_dir}" --ignore slicetiming --fs-license-file $FREESURFER_HOME/.license participant --participant-label "${SUBS[@]}" --output-space T1w template --fs-no-reconall --nthreads 4 --omp-nthreads 4 --mem-mb 32000 | tee -a ${log_file}
