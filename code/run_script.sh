...
if [[ $1 == '-h' ]] || [[ $1 == '-help' ]]; then
  ...
  echo "Usage: bash run_script.sh [-r] [-q] <scriptname> <sub> [<sub>]..."
  echo "       [-r]           runs script in interactive session (should qrsh first)"
  echo "       [-q]           runs script as a launched job (will launch as default)"
  ...
  run_cmd=""
  scripts=()
  if [[ $1 == '-r' ]] || [[ $1 == '-q' ]]; then run_cmd=$1; shift; fi
